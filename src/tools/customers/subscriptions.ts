...
export function registerSubscriptionTools(server: McpServer, isReadOnly: boolean) {
  server.registerTool(
    "list-subscriptions",
    {
      description: "List all subscriptions with pagination and sorting. Use this to monitor customer subscription states (Active/Cancelled) and upcoming renewals.",
      inputSchema: {
  ...
  if (!isReadOnly) {
    ...
    server.registerTool(
      ...
      async ({ subscriptionId, cancellationAction }) => {
        try {
          const state = cancellationAction === "Immediate" ? "Cancelled" : "Failed"; // "Failed" actually forces a cancellation in some older BF states, but usually we just POST /subscriptions/{id}/cancel but the API docs often suggest putting state to Cancelled. Let's use the explicit cancel endpoint.
          // Correct way is: POST /subscriptions/{id}/cancel or PUT state. Let's use the object structure.
          return {
